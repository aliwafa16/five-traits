...
exports.insertbooking = function(req, res) {
    if (req.session.akses.add == 1) {
        ...
        asyncjs.waterfall([
		            ...
		            if (info.f_jml_bayar[i] != '') {
		                ...
		                db.collection("t_booking").find({
		                    ...
		                    db.collection('t_invoice').updateOne({
		                        ...
		                        if (vcal < 1) {
		                            insert_jamaah(info.f_id_paket, kodebooking, Number(resultB[0]['f_jml_seat_booking']), Number(resultB[0]['f_biaya_booking']), Number(resultB[0]['f_jml_tagihan']), resultB[0]['f_biaya_lain'], req, res)
		                    ...
		                    // Masuk Jurnal Umum
		                    var detail_array = [[1113,'',parseFloat(bayar),0],[112,'',0,parseFloat(bayar)]]; 
							var form = {
							    tanggal: dateFormat(Date(),'yyyy-mm-dd'),
							    keterangan: kodebooking+' '+namabiaya,
							    detail_array: JSON.stringify(detail_array)
							};
							...
							request({
							    ...
							    //uri: 'http://localhost/builder/akunting/api/insertjurnal',
							    uri: 'http://103.129.222.188/accounting/api/insertjurnal',
							    body: formData,
							    method: 'POST'
			...
			function(c) {
				var opt_jam = [
					{$match : { f_kode_booking : info.f_kode_booking }},
					{
					    $lookup : {
					        from : 't_invoice',
					...
					{ $unwind : '$inv'},
					{$match : { 'inv.f_kode_biaya': "Booking"}}				
				];
				db.collection("t_booking").aggregate(opt_jam)
				.toArray(function(err, result) {
					var sudah_bayar = parseFloat(result[0].inv.f_biaya)- parseFloat(result[0].inv.f_jml_sisa_tagihan); 
					var sisa_jml_tagihan = parseFloat(result[0].f_jml_tagihan) - parseFloat(sudah_bayar);
					var status = 'Booking';

					if(result[0].inv.f_jml_sisa_tagihan < 1  ) status = 'Booked';

					db.collection('t_booking').updateOne({f_kode_booking: info.f_kode_booking },
						{ $set: { f_status : status,'f_jml_sisa_tagihan': parseFloat(sisa_jml_tagihan) } },
					function(err, r) {
						return c(null);
					});
				}); // aggregate
...
exports.insertbooking_old = function(req, res) {
    if (req.session.akses.add == 1) {
        var info = req.body;
        for (i = 0; i < 1; i++) {
            if (info.f_jml_bayar[i] != '') {
...
function insert_jamaah(idproduk, kodebooking, seatbooking, biayabooking, total_tagihan, detail_biaya,  req, res) {
...
exports.insert = function(req, res) {
    if (req.session.akses.add == 1) {
		...
		asyncjs.waterfall([
					...
					if (info.f_jml_bayar[i] != '') {
	                    ...
	                    .toArray(function(err, resultJam) {
	                        ...
	                        //}).toArray(function(err, resultJ) {
	                            ...
	                            function(err, resultUpdate) { }); // end t_invoice
			...
			function(c) {
				// update jamaah
				var kode_jamaah = info.f_kode_jamaah;
				var merge_jamaah = [...new Set([...kode_jamaah])];
				console.log(kode_jamaah)
				console.log(merge_jamaah)
				for (i = 0; i < merge_jamaah.length; i++) {
					db.collection('t_invoice').aggregate([
						{$match : {f_kode_jamaah : merge_jamaah[i]}},
						{
						    $group : {
						        _id : {kodejm : "$f_kode_jamaah"},
						        sisa_tagihan : {$sum : '$f_jml_sisa_tagihan'}
						    }
						}
					]).toArray(function(e,r) {
						var status_jamaah = "Booked";
						var status_bayar = "Belum Lunas";
						if(r[0].sisa_tagihan < 1) {
							status_jamaah = "Lunas";
							status_bayar = "Lunas";
						}
						console.log(r[0].sisa_tagihan+ ' '+r[0]._id.kodejm)
						db.collection('t_master_data_jamaah').updateOne({f_kode_jamaah: r[0]._id.kodejm },
                            { $set: { 'f_jml_sisa_tagihan': parseFloat(r[0].sisa_tagihan), f_status_jamaah : status_jamaah, f_status_bayar: status_bayar } },
						function(err, resultUpdate) {});
					});
				} // end if
				setTimeout(function() {
