	...
	<div class="container-fluid">
		<div class="card"> <!-- start .card -->
			...
			<div class="card-body"> <!-- start .card-body -->
				<form name="frmWork" id="frmWork" method="post" class="form-horizontal">
				...
				<div class="form-group row">
					...
					<div class="col-md-6"><input type="text" name="f_levelwork_desc" id="f_levelwork_desc" class="form-control" placeholder="Nama ..." value="" required></div>
				</div>
...
<?php if($fAct == 'Edit') { ;?>
	$.get("<?php echo site_url('work/fnWorkDataId?id='.$fId);?>",function(data){
		if(data)
		{
			$('#f_id').val(data.f_id);
			$('#f_account_id').val(data.f_account_id);
			$('#f_levelwork_desc').val(data.f_levelwork_desc);
			$('#f_aktif').val(data.f_aktif);
		}
	}, "json");

	var url = "<?php echo site_url('work/fnWorkUpdate?id='.$fId);?>";
<?php } ?>

<?php if($fAct == 'Add') { ;?>
	var url = "<?php echo site_url('work/fnWorkSave');?>";
<?php } ?>

	// Menyimpan data
	$('#frmWork').submit(function(e){
		var oForm = $('#frmWork');
		var rec = $('#frmWork').serialize();
