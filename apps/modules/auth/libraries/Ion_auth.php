...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		define('sAccount',$this->session->userdata('f_account_id'));
		define('sUsername',$this->session->userdata('username'));

		$trn_account = $this->db->select('f_language')->get_where('survey_setting',array('f_account_id'=> sAccount));
		if($trn_account->num_rows() > 0 ) { $setting = $trn_account->row(); $language = $setting->f_language; }
		else { $language = $this->config->item('language'); }
		
		if(get_cookie('act_language')) {
			if(file_exists(APPPATH.'language/'.get_cookie('act_language').'/act_lang.php') && is_dir(APPPATH.'language/'.get_cookie('act_language'))) {
				$this->lang->load('act', get_cookie('act_language'));
				define('LANGUAGE', get_cookie('act_language'));
			}
		} else {
			$this->lang->load('act', $language);
