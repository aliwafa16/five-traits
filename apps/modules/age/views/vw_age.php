	...
	<div class="container-fluid">
		<div class="card"> <!-- start .box -->
			<div class="card-header"> <!-- start .card-header -->
				<a class="btn btn-sm btn-primary" id="addAge"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
			</div> <!-- end .card-header -->
			
			<div class="card-body"> <!-- start .card-body -->
				<table id="tableAge" class="table datares">
					<thead>
						<tr>				
							<th>No</th>				
							<th>Name</th>				
...
$(document).ready(function(){
	oTable = $('#tableAge').DataTable({
		"order": [[ 0, "desc" ]],
		"processing": true,
		"serverSide": true,
		"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
		"ajax": {
            "url": "<?php echo site_url('age/fnAgeDataJson');?>",
            "type": "POST"
        },
		"columns": [
			...
			{ data: 'f_age_desc', name: 'f_age_desc' },
			{ data: 'f_age_aktif', name: 'f_age_aktif', className: 'text-center', render: vStatus },
			{ data: 'f_id', searchable: false, name: 'f_id',className: 'text-center' , orderable: false,
				render: function (data, type, row, meta) {
					var button = "";
					var toggle = 'off';var color_text = 'muted';
					if(row.f_age_aktif == 0) { toggle = 'on';color_text = 'success';}
					button += "&nbsp;<a href='javascript:void(0)' class='tip' title='Switch Active "+toggle+"' onclick='fnSwitch("+data+","+row.f_age_aktif+")'><i class='text-"+color_text+" fas fa-toggle-"+toggle+"'></i></a>&nbsp;&nbsp;"; 
	...
	$('#addAge').click(function()	{
		window.location.href = "<?php echo site_url('age/fnAgeAdd');?>";
	});
});

function fnEdit(idRow)
{
	window.location.href = "<?php echo site_url('age/fnAgeEdit');?>/"+idRow;
}
...
function fnSwitch(idRow,St)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('age/fnAgeSwitch');?>';
		if(result.value)
		{
...
function fnDelete(idRow)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('age/fnAgeDelete');?>';
		if(result.value)
		{
