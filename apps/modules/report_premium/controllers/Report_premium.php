...
class Report_premium extends MX_Controller
{
	function __construct()
	{
		...
		$this->load->model('mo_report_premium');
	...
	{
		$data = array(
			'app_title' => 'Data Reports Premium',
			'app_desc' => 'it all Reports'
		);
		$idAccount = sAccount;
		$search_account 			= $this->mo_report_premium->fnGetDataAccount();
		$listdemo 			= $this->mo_c->listDemo($idAccount);
		$event_account		= $this->mo_report_premium->comboDataEvent();
	...
	public function fnReportsData()
	{
		...
		//opn($paramGroup);die();
		/*---------- Data Responden ----------*/
		$data_survey			= $this->mo_report_premium->getTrxSurvey($eventId, $paramGroup, null);

		/*---------- Data Category ----------*/
		$variabel 				= $this->mo_report_premium->getVariabel();
		//$dimensi 				= $this->mo_report_premium->getDimensi();
		$about_item				= $this->mo_report_premium->getItem();
		//opn($about_item);
		...
		/*---------- Data Survey ----------*/
		$sr_result = $this->mo_report_premium->fnSurveyTotal($data_survey, $about_item);
		//$sumprio = $this->mo_report_premium->olahDataSummaryQuesioner($data_survey,$variabel,$dimensi,$about_item,$totalResponden);
	...
	public function fnReportsDataId()
	{
		...
		$data	= $this->mo_report_premium->getDataId($id);
	...
	{
		$id 	= $this->input->get_post('id');
		if ($this->mo_report_premium->deleteData($id)) {
	...
	{
		$json['result'] = $this->mo_report_premium->remoteComboData();
	...
	public function fnReportsExcel()
	{
		...
		$idAccount = sAccount;
		$search_account 			= $this->mo_report_premium->fnGetDataAccount();
		$values_account 			= $this->mo_report_premium->comboDataValues();
		///*---------- Name Label ----------*
