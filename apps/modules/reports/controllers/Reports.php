...
class Reports extends MX_Controller
{
	...
	{
		$data = array(
			'app_title' => 'Data Reports Utilitarian',
			'app_desc' => 'it all Reports'
		);
		$idAccount = sAccount;
		$search_account 			= $this->mo_reports->fnGetDataAccount();
		$listdemo 			= $this->mo_c->listDemo($idAccount);
		$event_account		= $this->mo_reports->comboDataEvent();
		...
		$data['side_main']	= 'reports';
		$data['content']	= 'vw_reports';
		$this->load->view('admin/main', $data);
	}

	public function fnReportsDataUtilitarian()
	{
		$event_id = $this->input->get('event_account');
		// Get event
		$event = $this->db->get_where('t_event', ['f_event_id' => $event_id])->row_array();
		$variabel_id = $event['f_variabel_id'];
		$dimensi = $this->db->get_where('t_dimensi', ['f_variabel_id' => $variabel_id])->result_array();

		$pernyataan = $this->db->get_where('t_item_pernyataan', ['f_variabel_id' => $variabel_id])->result_array();
		$responden = $this->db->get_where('trn_survey_empex', ['f_event_id' => $event_id])->result_array();
		$jumlah_responden = count($responden);
		$mean = array();
		$stdev_data = array();
		$stdev = array();
		$left = array();
		...
		foreach ($responden as $row) {
			$jawaban_responden = json_decode($row['f_survey'], true);
			$item_pernyataan = $jawaban_responden['item_pertanyaan'];
			// print("<pre>" . print_r($jawaban_responden, true) . "</pre>");



			foreach ($item_pernyataan as $key => $value) {
				$responden[$responden_loop][$key] = $value;
				$mean[$key] += $value / $jumlah_responden;
				$stdev_data[$key][] = $value;
			}
			$responden_loop++;
		}

		foreach ($stdev_data as $key => $value) {
			$stdev[$key] = decimal_report($this->hitungStandarDeviasiSample($value), 2);
		}

		foreach ($stdev as $key => $value) {
			$left[$key] = decimal_report($mean[$key] - $value, 2);
			$right[$key] = decimal_report($mean[$key] + $value, 2);
		}


		// DEMOGRAFI

		$this->db->select('COUNT(responden.f_jenis_kelamin) as y, gender.f_gender_name as name, concat(round((100*count(responden.f_jenis_kelamin))/' . $jumlah_responden . ',0),"%") as persentase');
		$this->db->from('trn_survey_empex as responden');
		$this->db->join('table_gender as gender', 'responden.f_jenis_kelamin=gender.f_gender_id');
		$this->db->group_by('responden.f_jenis_kelamin');
		$this->db->where('responden.f_event_id', $event_id);
		$gender = $this->db->get()->result_array();


		$this->db->select('COUNT(responden.f_umur) as y, age.f_age_desc as name, concat(round((100*count(responden.f_umur))/' . $jumlah_responden . ',0),"%") as persentase');
		$this->db->from('trn_survey_empex as responden');
		$this->db->join('table_age as age', 'responden.f_umur=age.f_id');
		$this->db->group_by('responden.f_umur');
		$this->db->where('responden.f_event_id', $event_id);
		$umur = $this->db->get()->result_array();

		$this->db->select('COUNT(responden.f_pekerjaan) as y, work.f_pekerjaan_desc as name, concat(round((100*count(responden.f_pekerjaan))/' . $jumlah_responden . ',0),"%") as persentase');
		$this->db->from('trn_survey_empex as responden');
		$this->db->join('table_pekerjaan as work', 'responden.f_pekerjaan=work.f_id');
		$this->db->group_by('responden.f_pekerjaan');
		$this->db->where('responden.f_event_id', $event_id);
		$pekerjaan = $this->db->get()->result_array();


		$this->db->select('COUNT(responden.f_pendapatan) as y, pendapatan.f_pendapatan_desc as name, concat(round((100*count(responden.f_pendapatan))/' . $jumlah_responden . ',0),"%") as persentase');
		$this->db->from('trn_survey_empex as responden');
		$this->db->join('table_pendapatan as pendapatan', 'responden.f_pendapatan=pendapatan.f_id');
		$this->db->group_by('responden.f_pendapatan');
		$this->db->where('responden.f_event_id', $event_id);
		$pendapatan = $this->db->get()->result_array();

		$eventId		= $this->input->get_post('event_account');
		$paramGroup = array();	// Group yang difilter
		$data_survey	= $this->mo_reports->getTrxSurvey($eventId, $paramGroup, null);

		/*---------- Data Category ----------*/
		$variabel 				= $this->mo_reports->getVariabel();
		//$dimensi 				= $this->mo_reports->getDimensi();
		$about_item				= $this->mo_reports->getItem();
		//opn($about_item);
		...
		/*---------- Data Survey ----------*/
		$sr_result = $this->mo_reports->fnSurveyTotal($data_survey, $about_item);

		//$sumprio = $this->mo_reports->olahDataSummaryQuesioner($data_survey,$variabel,$dimensi,$about_item,$totalResponden);
		...
		// print("<pre>" . print_r($data_final, true) . "</pre>");
		// die;


		$this->load->view('vw_reports_data', $data_final, $vars);


		// print("<pre>" . print_r($left, true) . "</pre>");
		// print("<pre>" . print_r($right, true) . "</pre>");


		// print("<pre>" . print_r($stdev, true) . "</pre>");
		// print("<pre>" . print_r($mean, true) . "</pre>");
		// print("<pre>" . print_r($responden, true) . "</pre>");
		// print("<pre>" . print_r($stdev_data, true) . "</pre>");
	}
	...
	public function fnReportsDataId()
	{
		...
		$data	= $this->mo_reports->getDataId($id);
	...
	{
		$id 	= $this->input->get_post('id');
		if ($this->mo_reports->deleteData($id)) {
	...
	{
		$json['result'] = $this->mo_reports->remoteComboData();
	...
	function hitungStandarDeviasiPopulasi($data)
	{
		// Langkah 1: Hitung nilai rata-rata
		$rataRata = array_sum($data) / count($data);

		// Langkah 2: Hitung selisih antara setiap data dengan nilai rata-rata
		$selisihKuadrat = array_map(
			function ($x) use ($rataRata) {
		...
		$jumlahKuadratSelisih = array_sum($selisihKuadrat);

		// Langkah 4: Bagi jumlah hasil kuadrat selisih dengan jumlah total data (n)
		$n = count($data);
		$standarDeviasiPopulasi = sqrt($jumlahKuadratSelisih / $n);

		return $standarDeviasiPopulasi;
	}

	function hitungStandarDeviasiSample($data)
	{
		// Langkah 1: Hitung nilai rata-rata
		$rataRata = array_sum($data) / count($data);
		...
		// Langkah 4: Bagi jumlah hasil kuadrat selisih dengan (n-1) untuk sampel
		$n = count($data);
		$standarDeviasiSample = sqrt($jumlahKuadratSelisih / ($n - 1));

		return $standarDeviasiSample;
	}
	...
	public function fnReportsExcel()
	{
		...
		$idAccount = sAccount;
		$search_account 			= $this->mo_reports->fnGetDataAccount();
		$values_account 			= $this->mo_reports->comboDataValues();
		///*---------- Name Label ----------*
