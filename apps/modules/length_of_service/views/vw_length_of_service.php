	...
	<div class="container-fluid">
		<div class="card"> <!-- start .box -->
			<div class="card-header"> <!-- start .card-header -->
				<a class="btn btn-sm btn-primary" id="addLength_of_service"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
			</div> <!-- end .card-header -->
			
			<div class="card-body"> <!-- start .card-body -->
				<table id="tableLength_of_service" class="table datares">
					<thead>
						<tr>				
							<th>No</th>				
							<th>Nama</th>				
...
$(document).ready(function(){
	oTable = $('#tableLength_of_service').DataTable({
		"order": [[ 0, "desc" ]],
		"processing": true,
		"serverSide": true,
		"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
		"ajax": {
            "url": "<?php echo site_url('length_of_service/fnLength_of_serviceDataJson');?>",
            "type": "POST"
        },
		"columns": [
			...
			{ data: 'f_service_desc', name: 'f_service_desc' },
			{ data: 'f_service_aktif', name: 'f_service_aktif', className: 'text-center', render: vStatus },
			{ data: 'f_id', searchable: false, name: 'f_id',className: 'text-center' , orderable: false,
				render: function (data, type, row, meta) {
					var button = "";
					var toggle = 'off';var color_text = 'muted';
					if(row.f_service_aktif == 0) { toggle = 'on';color_text = 'success';}
					button += "&nbsp;<a href='javascript:void(0)' class='tip' title='Switch Active "+toggle+"' onclick='fnSwitch("+data+","+row.f_service_aktif+")'><i class='text-"+color_text+" fas fa-toggle-"+toggle+"'></i></a>&nbsp;&nbsp;"; 
	...
	$('#addLength_of_service').click(function()	{
		window.location.href = "<?php echo site_url('length_of_service/fnLength_of_serviceAdd');?>";
	});
});

function fnEdit(idRow)
{
	window.location.href = "<?php echo site_url('length_of_service/fnLength_of_serviceEdit');?>/"+idRow;
}
...
function fnSwitch(idRow,St)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('length_of_service/fnLength_of_serviceSwitch');?>';
		if(result.value)
		{
...
function fnDelete(idRow)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('length_of_service/fnLength_of_serviceDelete');?>';
		if(result.value)
		{
