	...
	<div class="container-fluid">
		<div class="card"> <!-- start .box -->
			<div class="card-header"> <!-- start .card-header -->
				<a class="btn btn-sm btn-primary" id="addPosition5"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
			</div> <!-- end .card-header -->
			
			<div class="card-body"> <!-- start .card-body -->
				<table id="tablePosition5" class="table datares">
					<thead>
						<tr>				
							<th>No</th>				
							<th>Level 4</th>				
...
$(document).ready(function(){
	oTable = $('#tablePosition5').DataTable({
		"order": [[ 0, "desc" ]],
		"processing": true,
		"serverSide": true,
		"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
		"ajax": {
            "url": "<?php echo site_url('position5/fnPosition5DataJson');?>",
            "type": "POST"
        },
		"columns": [
			...
			{ data: 'f_id', searchable: false, name: 'f_id',className: 'center' , orderable: false,
				render: function (data, type, row, meta) {
					var button = "";
	...
	$('#addPosition5').click(function()	{
		window.location.href = "<?php echo site_url('position5/fnPosition5Add');?>";
	});
});

function fnEdit(idRow)
{
	window.location.href = "<?php echo site_url('position5/fnPosition5Edit');?>/"+idRow;
}
...
function fnSwitch(idRow,St)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('position5/fnPosition5Switch');?>';
		if(result.value)
		{
...
function fnDelete(idRow)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('position5/fnPosition5Delete');?>';
		if(result.value)
		{
