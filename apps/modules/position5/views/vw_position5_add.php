	...
	<div class="container-fluid">
		<div class="card"> <!-- start .card -->
			...
			<div class="card-body"> <!-- start .card-body -->
				<form name="frmPosition5" id="frmPosition5" method="post" class="form-horizontal">
				<div class="form-group row">
					<label class="col-md-2" style="padding-top:7px;">Level 4 <span class="text-red">*</span></label>
					<div class="col-md-6">
					<?php
						echo form_dropdown('f_id4',false,false,'id="f_id4" class="form-control select" style="width: 100%" required');
					?>
					</div>
				</div>
				<div class="form-group row">
...
$(document).ready(function(){
    ...
    $f_id4.select2({
		placeholder: "-- Pilih ..",
		ajax: {
			url 		: "<?php echo site_url('position4/fnPosition4ComboData');?>", // << Example
			dataType	: 'json',
			delay		: 250,
...
<?php if($fAct == 'Edit') { ;?>
	$.get("<?php echo site_url('position5/fnPosition5DataId?id='.$fId);?>",function(data){
		if(data)
		{
			var $option1 = $('<option value="'+data.f_id4+'" selected="selected">'+data.f_desc4+'</option>');
			$('#f_id4').append($option1).trigger('change');
	...
	var url = "<?php echo site_url('position5/fnPosition5Update?id='.$fId);?>";
<?php } ?>

<?php if($fAct == 'Add') { ;?>
	var url = "<?php echo site_url('position5/fnPosition5Save');?>";
<?php } ?>

	// Menyimpan data
	$('#frmPosition5').submit(function(e){
		var oForm = $('#frmPosition5');
		var rec = $('#frmPosition5').serialize();
		...
		if($(this).valid()) {
			$.post(url, rec, function(msg){
				if(msg.msg)
				{
					window.location.href = "<?php echo site_url('position5');?>";
				}
				else
				{
