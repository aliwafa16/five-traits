...
<section class="content">
	<div class="container-fluid">
		<div class="card">
			...
			<div class="card-body">
				<!-- start .card-body -->
				<form name="frmRegion" id="frmRegion" method="post" class="form-horizontal">
					...
					<div class="form-group row">
						...
						<div class="col-md-6"><input type="text" name="f_region_name" id="f_region_name" class="form-control" placeholder="Nama ..." value="" required></div>
					</div>
					<input type="hidden" name="f_region_aktif" id="f_region_aktif" value="">
...
<script>
	...
	$(document).ready(function() {
		...
		<?php if ($fAct == 'Edit') {; ?>
			$.get("<?php echo site_url('region/fnRegionDataId?id=' . $fId); ?>", function(data) {
				if (data) {
					$('#f_id').val(data.f_id);
					$('#f_account_id').val(data.f_account_id);
					$('#f_region_name').val(data.f_region_name);
					$('#f_region_aktif').val(data.f_region_aktif);
				}
			}, "json");

			var url = "<?php echo site_url('region/fnRegionUpdate?id=' . $fId); ?>";
		<?php } ?>

		<?php if ($fAct == 'Add') {; ?>
			var url = "<?php echo site_url('region/fnRegionSave'); ?>";
		<?php } ?>

		// Menyimpan data
		$('#frmRegion').submit(function(e) {
			var oForm = $('#frmRegion');
			var rec = $('#frmRegion').serialize();
			...
			if ($(this).valid()) {
				$.post(url, rec, function(msg) {
					if (msg.msg) {
						window.location.href = "<?php echo site_url('region'); ?>";
					} else {
						swal.fire({
							text: 'Gagal !!',
							icon: 'error',
