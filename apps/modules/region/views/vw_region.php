	...
	<div class="container-fluid">
		<div class="card"> <!-- start .box -->
			<div class="card-header"> <!-- start .card-header -->
				<a class="btn btn-sm btn-primary" id="addRegion"><i class="fa fa-plus"></i> Tambah</a>&nbsp;&nbsp;
			</div> <!-- end .card-header -->
			
			<div class="card-body"> <!-- start .card-body -->
				<table id="tableRegion" class="table datares">
					<thead>
						<tr>				
							<th>No</th>				
							<th>Nama</th>				
...
$(document).ready(function(){
	oTable = $('#tableRegion').DataTable({
		"order": [[ 0, "desc" ]],
		"processing": true,
		"serverSide": true,
		"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
		"ajax": {
            "url": "<?php echo site_url('region/fnRegionDataJson');?>",
            "type": "POST"
        },
		"columns": [
			...
			{ data: 'f_region_name', name: 'f_region_name' },
			{ data: 'f_region_aktif', name: 'f_region_aktif',className: 'text-center', render: vStatus },
			{ data: 'f_id', searchable: false, name: 'f_id',className: 'text-center' , orderable: false,
				render: function (data, type, row, meta) {
					var button = "";
					var toggle = 'off';var color_text = 'muted';
					if(row.f_region_aktif == 0) { toggle = 'on';color_text = 'success';}
					button += "&nbsp;<a href='javascript:void(0)' class='tip' title='Switch Active "+toggle+"' onclick='fnSwitch("+data+","+row.f_region_aktif+")'><i class='text-"+color_text+" fas fa-toggle-"+toggle+"'></i></a>&nbsp;&nbsp;"; 
	...
	$('#addRegion').click(function()	{
		window.location.href = "<?php echo site_url('region/fnRegionAdd');?>";
	});
});

function fnEdit(idRow)
{
	window.location.href = "<?php echo site_url('region/fnRegionEdit');?>/"+idRow;
}
...
function fnSwitch(idRow,St)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('region/fnRegionSwitch');?>';
		if(result.value)
		{
...
function fnDelete(idRow)
{
	Swal.fire({
		...
		var urlUpdate='<?php echo site_url('region/fnRegionDelete');?>';
		if(result.value)
		{
