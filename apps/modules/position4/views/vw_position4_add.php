	...
	<div class="container-fluid">
		<div class="card"> <!-- start .card -->
			...
			<div class="card-body"> <!-- start .card-body -->
				<form name="frmPosition4" id="frmPosition4" method="post" class="form-horizontal">
				<div class="form-group row">
					<label class="col-md-2" style="padding-top:7px;">Level 3 <span class="text-red">*</span></label>
					<div class="col-md-6">
					<?php
						echo form_dropdown('f_id3',false,false,'id="f_id3" class="form-control select" style="width: 100%" required');
					?>
					</div>
				</div>
				<div class="form-group row">
...
$(document).ready(function(){
    ...
    $f_id3.select2({
		placeholder: "-- Pilih ..",
		ajax: {
			url 		: "<?php echo site_url('position3/fnPosition3ComboData');?>", // << Example
			dataType	: 'json',
			delay		: 250,
...
<?php if($fAct == 'Edit') { ;?>
	$.get("<?php echo site_url('position4/fnPosition4DataId?id='.$fId);?>",function(data){
		if(data)
		{
			var $option1 = $('<option value="'+data.f_id3+'" selected="selected">'+data.f_desc3+'</option>');
			$('#f_id3').append($option1).trigger('change');
			$('#f_position_desc').val(data.f_position_desc);
			$('#f_total').val(data.f_total);
		}
	}, "json");

	var url = "<?php echo site_url('position4/fnPosition4Update?id='.$fId);?>";
<?php } ?>

<?php if($fAct == 'Add') { ;?>
	var url = "<?php echo site_url('position4/fnPosition4Save');?>";
<?php } ?>

	// Menyimpan data
	$('#frmPosition4').submit(function(e){
		var oForm = $('#frmPosition4');
		var rec = $('#frmPosition4').serialize();
		...
		if($(this).valid()) {
			$.post(url, rec, function(msg){
				if(msg.msg)
				{
					window.location.href = "<?php echo site_url('position4');?>";
				}
				else
				{
